...
const RowCounter = memo(({ title, isSelectChange }: Props) => {
    ...
    const comparisonDecreaseCounter = getComparisonCounter(
        title,
        'decrease',
        properties[title]
    )
    const comparisonIncreaseCounter = getComparisonCounter(
        title,
        'increase',
        properties[title]
    )
    const conditionForCounter =
        title === 'children' ? properties[title].length : properties[title]
