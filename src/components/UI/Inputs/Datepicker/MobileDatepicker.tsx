        ...
        <>
            <DatePicker
                ...
                onChange={(update: Date | null) =>
                    handleSetDateRange([update, endDate])
                }
                selectsStart
                startDate={startDate}
                endDate={endDate}
                ...
                onCalendarOpen={handleCalendarOpen({
                    borderStyle: `border: 3px solid ${({ theme }: any) =>
                        theme.colors.acceptBlue};`,
                    childWrapper: 1,
                })}
            />
            <DatePicker
                selected={endDate}
                onChange={(update: Date | null) =>
                    handleSetDateRange([startDate, update])
                }
                selectsEnd
                startDate={startDate}
                endDate={endDate}
                ...
                onCalendarOpen={handleCalendarOpen({
                    borderStyle: css`
                        border: 3px solid
                            ${({ theme }: any) => theme.colors.acceptBlue};
                    `,
                    childWrapper: 2,
                })}
            />
